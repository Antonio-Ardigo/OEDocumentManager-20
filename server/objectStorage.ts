...
export class ObjectStorageService {
  ...
  async getDocumentUploadURL(fileName: string): Promise<string> {
    ...
    const fullPath = `${privateObjectDir}/documents/${documentId}/${fileName}`;
  ...
  async getDocumentFile(objectPath: string): Promise<File> {
    ...
    const documentPath = parts.slice(1).join("/");
    let privateDir = this.getPrivateObjectDir();
    if (!privateDir.endsWith("/")) {
      privateDir = `${privateDir}/`;
    }
    const fullDocumentPath = `${privateDir}${documentPath}`;
    const { bucketName, objectName } = parseObjectPath(fullDocumentPath);
    const bucket = objectStorageClient.bucket(bucketName);
